...
1. デモページを開きます。
2. **「追加」** を押します。
3. 攻略HTMLファイル、または攻略サイトURLを選びます。
4. タイトル・プラットフォーム・タグ・メモ・ジャケット画像などを必要に応じて設定します。
5. ライブラリから攻略を開いて使用します。大切なデータは定期的にバックアップしてください。

動作確認用のHTMLは [`sample-walkthrough.html`](sample-walkthrough.html) にあります。
